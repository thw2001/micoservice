...
om_status_t om_publish(om_topic_t* topic, void* buff, uint32_t size, bool block,
  ...
  om_msg_t msg = {.buff = buff, .size = size, .topic = topic};
...
om_status_t om_suber_export_with_len(om_suber_t* suber, void* buff, uint32_t* len, bool in_isr) {
  OM_ASSERT(suber);
  OM_ASSERT(suber->mode == OM_SUBER_MODE_EXPORT);
