...
// 这个选项是为了发布消息时可以直接用传进来的消息体，
// 而不是拷贝一份到topic缓存区中，可以减少一次拷贝
/*
使用方法：
om_topic = om_config_topic(NULL, "a", topic);

// 发送时
om_topic->buff_len = payload_len; // 手动修改缓存区长度，因为发送用的缓存区不再是topic的了
  ...
  for (index = (const uint8_t*)format; *index != '\0'; index++) {
    ...
    switch (GET_CAPITAL(*index)) {
      ...
      case STATIC_CACHE_FLAG:
        ...
        topic->virtual_mode = false;
        break;
      case USE_TMP_CACHE_FLAG:
        topic->msg.buff = va_arg(valist, void*);
        // virtual_mode为true就是使用虚的msg结构体
