...
static void __om_list_add(om_list_head_t* new_data, om_list_head_t* prev,
                       om_list_head_t* next) {
  new_data->next = next;
  prev->next = new_data;
}
...
void om_list_add(om_list_head_t* new_data, om_list_head_t* head) {
  __om_list_add(new_data, head, head->next);
}

static void __om_list_del(om_list_head_t* prev, om_list_head_t* next) {
  prev->next = next;
}
...
void om_list_del(om_list_head_t* entry) {
  __om_list_del(__list_get_prev(entry), entry->next);
}
